...
function renderExecToolGroup(record) {
  ...
  const pairs = Array.from({ length: commands.length }, (unused, index) =>
    renderExecCommandPair(commands[index], parsedOutput.results[index], index, commands.length, record)
  );
  ...
  const body = [
    ...
    !commands.length && !updatePlans.length ? renderLazyCodeDetails("Raw exec input", input, "javascript", `record-${record.line}:raw-input`) : ""
  ].filter(Boolean).join("\n");
  const title = [
    "exec",
    commands.length ? `${formatNumber(commands.length)} command${commands.length === 1 ? "" : "s"}` : "",
    updatePlans.length ? `${formatNumber(updatePlans.length)} plan${updatePlans.length === 1 ? "" : "s"}` : "",
