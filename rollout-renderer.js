  ...
  for (const record of records) {
    ...
    if (record.value?.type === "event_msg" && payloadType === "patch_apply_end") {
      const group = [...pendingGroups].reverse().find(candidate => {
        const callPayload = candidate.callRecord.value?.payload ?? {};
        return isToolName(callPayload.name, "exec") && String(callPayload.input ?? callPayload.arguments ?? "").includes("apply_patch");
      });
      if (group) {
        group.eventRecords.push(record);
        consumedRecords.add(record);
      }
    }
  }
...
function readJsStringProperty(objectText, propertyName) {
  ...
  let escaped = false;
  for (let index = start + 1; index < objectText.length; index += 1) {
    ...
    if (escaped) {
      ...
      return decodeJsStringLiteral(objectText.slice(start, index + 1));
    }
  }
  return "";
}
...
function renderExecCommandPair(command, result, index, total, record) {
  ...
  return `
    <section class="rollout-exec-command ${className}">
      <div class="rollout-exec-command-head">
        <span>${escapeHtml(command?.tool || "exec_command")} ${formatNumber(index + 1)}/${formatNumber(total)}</span>
...
function renderAttachedPatchEvent(patchRecord) {
  ...
  return `
    <section class="rollout-exec-command ${payload.success === false ? "is-error" : ""}">
      <div class="rollout-exec-command-head">
        <span>patch_apply</span>
        <span>${escapeHtml([payload.status, payload.success === false ? "failed" : payload.success === true ? "success" : ""].filter(Boolean).join(" / "))}</span>
      </div>
...
function renderExecToolGroup(record) {
  ...
  const parsedOutput = parseStructuredToolOutput(outputPayload.output);
  const total = Math.max(commands.length, parsedOutput.results.length);
  const pairs = Array.from({ length: total }, (unused, index) =>
    renderExecCommandPair(commands[index], parsedOutput.results[index], index, total, record)
  );
  const patches = group.eventRecords.map(renderAttachedPatchEvent);
  const supplementalOutput = parsedOutput.plainText
    .filter(text => !/^Script (?:completed|running)\b/i.test(text.trim()) && !/^(?:\{\}|\[\])$/.test(text.trim()))
    .join("\n");
  const failed = parsedOutput.results.some(result => Number(result.exit_code) !== 0)
    || group.eventRecords.some(eventRecord => eventRecord.value?.payload?.success === false);
  const body = [
    renderKeyValueList([
      ["Tool", callPayload.name || "exec"],
      ["Call ID", callPayload.call_id],
      ["Status", callPayload.status],
      ["Commands", commands.length || null],
      ["Patches", patches.length || null]
    ]),
    ...
    !commands.length ? renderLazyCodeDetails("Raw exec input", input, "javascript", `record-${record.line}:raw-input`) : ""
  ].filter(Boolean).join("\n");
  const title = [
    "exec",
    commands.length ? `${formatNumber(commands.length)} command${commands.length === 1 ? "" : "s"}` : "",
    patches.length ? `${formatNumber(patches.length)} patch${patches.length === 1 ? "" : "es"}` : ""
  ].filter(Boolean).join(" / ");
